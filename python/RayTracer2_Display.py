    ...
    def absorption_table(self,rows=40,fancy=True):
        ...
        for n in range(np.shape(self.output[1])[2]):
            for i in range(np.shape(self.output[1])[0]):
                data = np.array([n,i,self.output[1][i][0][n][0],self.output[1][i][0][n][1],self.output[1][i][1][n][0],\
        ...
        # creates a nice looking table, but not very useful for calling data from
        if fancy == True:
            absorption_fancy=pd.DataFrame(absorption_list,columns=['surface','scatter number','surface absorption (SA)', \
        'SA orientation','bulk absorption (BA)','BA orientation','escaped geometry (EG)','EG orientation',\
            'dropped below threshold (BT)','BT orientation','still following (SF)','SF orientation'])
        # This is all just styling to make it look a little nicer
            d=dict(selector="th",props=[('text-align', 'center')])
            absorption_df = absorption_fancy.round(4).head(rows).style.set_table_styles([d]).format({'surface' : "{:.0f}",\
            'scatter number' : "{:.0f}",'surface absorption (SA)' : lambda x: "0" if x==0 else x,'SA orientation' : \
            lambda x: "0" if x==0 else x,'bulk absorption (BA)' : lambda x: "0" if x==0 else x,'BA orientation' : \
            lambda x: "0" if x==0 else x,'escaped geometry (EG)' : lambda x: "0" if x==0 else x,'EG orientation' : \
            lambda x: "0" if x==0 else x,'dropped below threshold (BT)' : lambda x: "0" if x==0 else x,'BT orientation' :\
            lambda x: "0" if x==0 else x,'still following (SF)' : lambda x: "0" if x==0 else x,'SF orientation' : lambda x: \
            "0" if x==0 else x,}).set_caption('Absorption Table')
        ...
        #creates the much more useful (for external functions) dataframe
        if fancy == False:
            absorption_raw=pd.DataFrame(absorption_list,columns=['surface','scatter number','surface absorption (SA)', \
        'SA orientation','bulk absorption (BA)','BA orientation','escaped geometry (EG)','EG orientation',\
            'dropped below threshold (BT)','BT orientation','still following (SF)','SF orientation'])
